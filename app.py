...
with st.sidebar:
    ...
    api_key = st.text_input(
        ...
        help="Get a key at openrouter.ai/keys (starts with 'sk-or-v1-'). Loaded from .env if set there.",
    )
    if not api_key:
        st.caption("\u26A0\uFE0F No API key set — add one above or in a `.env` file (`OPENAI_API_KEY=...`).")
...
if pending_text:
    ...
    if not already_done:
        ...
        if analyze_clicked:
            if not api_key:
                st.error("No OpenRouter API key set. Add one in the sidebar first.")
            else:
...
if analysis:
    ...
    # -----------------------------------------------------------------------
    # Quick Insights — zero extra API cost, computed entirely from data the
    # analysis already produced (see insights.py). No LLM call involved.
    # -----------------------------------------------------------------------
    ...
    readiness = get_readiness_status(analysis.get("compliance", []))
    ...
    clarification_qs = get_clarification_questions(analysis.get("compliance", []))
    if clarification_qs:
        ...
        for cat in CATEGORY_ORDER:
            ...
            with st.expander(f"{meta['emoji']} {meta['title']} ({len(cat_items)} items)", expanded=True):
                ...
                for it in cat_items:
                    ...
                    status_badge = f"<span style=\"background:{color}22; color:{color}; padding:2px 8px; border-radius:5px; font-size:11px; font-weight:700;\">{status}</span>"
                    if status == "REVIEW" and gap_type:
                        status_badge += f"<br><span style='font-size:10px; color:#999; font-style:italic;'>{gap_type}</span>"
    ...
    with tabs[6]:
        ...
        if not questions:
            ...
            if st.session_state.generated_responses is None:
                ...
                st.markdown("#### \U0001F4DD Draft Responses (from company knowledge base)")
                st.caption(
