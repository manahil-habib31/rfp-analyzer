...
def build_markdown_report(analysis: dict, source_label: str) -> str:
    ...
    if questions:
        ...
        lines.append("")
    lines.append("---")
    lines.append("*Generated by RFP Analyzer · OpenAI GPT-4o mini · Verify all terms against the original RFP before submission.*")
    return "\n".join(lines)
...
if pending_text:
    ...
    if not already_done:
        ...
        if analyze_clicked:
            if not api_key:
                st.error("No OpenAI API key set. Add one in the sidebar first.")
            else:
